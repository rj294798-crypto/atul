    ...
    <section className="max-container">
      ...
      <div className="my-4">
        <p className="text-slate-500 mt-2 leading-relaxed">
          ...
          some DevOps tasks that shaped me into a versatile full-stack
          developer.
        </p>
        <p className="text-slate-500 mt-2 leading-relaxed">
          This blend of expertise enables me to quickly adapt to new
          technologies and deliver efficient, user-centered solutions tailored
          to diverse client needs.
        </p>
        <p className="text-slate-500 mt-2 leading-relaxed">
          Discover below some of the projects I’ve had the opportunity to work
          on:
        </p>
      </div>
        ...
        {projects.map((project, index) => (
          <div className="lg:w-[400px] w-full" key={project.name}>
            <div className="mt-5 flex flex-col">
              <h4 className="text-2xl font-poppins font-semibold mb-1">
                {project.name}
              </h4>
              <Carousel
                key={`slider-${index}`}
              ...
              >
                {project.imageList?.map((img, imgIndex) => (
                  <div>
                    <img
                      src={img}
                      alt={`image-${project.name}-${index}`}
                      key={`slider-img-${index}-${imgIndex}`}
                    />
                  </div>
                ))}
              </Carousel>
